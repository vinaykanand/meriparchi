          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4 w-full">
            ...
            {lookupData && (
              <div className="flex gap-2 w-full md:w-auto md:ml-auto">
                ...
                <a href={`/dashboard/user/payments?phone=${searchedLookupPhone}`} className="flex-1 md:flex-none text-center px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 transition-colors shadow-sm shadow-green-500/20">
                  ₹ Log Payment
                </a>
              </div>
            )}
                ...
                <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    <thead className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold uppercase tracking-wider text-xs">
                      <tr>
                        <th className="px-2 py-1 w-[90px]">Time</th>
                        <th className="px-2 py-1 w-[60px]">No</th>
                        <th className="px-2 py-1">Item</th>
                        <th className="px-2 py-1 w-[50px] text-right">Qty</th>
                        <th className="px-2 py-1 w-[70px] text-right">Rate</th>
                        <th className="px-2 py-1 w-[80px] text-right">Amt</th>
                      </tr>
                    </thead>
                        ...
                        return displaySlips.map((s: any, i: number) => {
                          ...
                          return (
                            <React.Fragment key={i}>
                              ...
                              >
                            <td className="px-2 py-0.5 text-slate-500 text-xs">
                              {isFirst ? new Date(s.time).toLocaleDateString() : ""}
                            </td>
                            <td className="px-2 py-0.5 font-semibold text-blue-600 dark:text-blue-400 text-xs">
                              {isFirst ? `#${s.no}` : ""}
                            </td>
                            ...
                            <td className={`px-2 py-0.5 font-semibold text-right text-xs ${parseFloat(s.qty) < 0 ? 'text-red-500' : 'text-slate-700 dark:text-slate-300'}`}>{s.qty}</td>
                            <td className="px-2 py-0.5 text-slate-500 text-right text-xs">₹{s.rate}</td>
                          ...
                          {isLast && (
                            <tr className="bg-slate-50 dark:bg-slate-800/30 group">
                              ...
                              <td className={`px-2 py-1 font-bold border-t border-slate-200 dark:border-slate-700 text-right text-xs ${slipTotals[s.no] < 0 ? 'text-red-500' : 'text-slate-900 dark:text-slate-100'}`}>
