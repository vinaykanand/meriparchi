...
export default function TransactionTypesPage() {
  ...
  const handleAddType = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCode.trim() || !newName.trim() || !session?.orgcode) return;
    try {
      setSubmitting(true);
      setErrorMsg("");
      setSuccessMsg("");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {editingType ? (
            <div className="bg-white dark:bg-slate-800/50 border border-amber-200 dark:border-amber-900/50 rounded-3xl p-6 shadow-sm sticky top-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold flex items-center gap-2 text-amber-600 dark:text-amber-400">
                  <PencilSquareIcon className="w-5 h-5" /> Edit Transaction Type
                </h3>
            ...
            <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-3xl p-6 shadow-sm sticky top-6">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                <PlusIcon className="w-5 h-5 text-blue-600 dark:text-blue-400" /> Create Type
              </h3>

              <form onSubmit={handleAddType} className="flex flex-col gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Code</label>
                  <input type="text" placeholder="e.g. WH_TRANS" value={newCode} onChange={(e) => setNewCode(e.target.value)} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 font-semibold text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Name</label>
                ...
                <button type="submit" disabled={submitting || !newCode.trim() || !newName.trim()} className="w-full p-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors text-sm disabled:opacity-50">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h3 className="text-lg font-bold">Transaction Type Registry</h3>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40">
                  {filteredTypes.map((t) => (
                    <tr key={t.code} className="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-sm font-semibold">{t.name}</td>
